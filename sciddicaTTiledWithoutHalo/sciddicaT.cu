...
__global__ void sciddicaTFlowsComputation(int r, int c, double nodata, double *Sz, double *Sh, double *Sf, double p_r, double p_epsilon){
    ...
    if(i > 0 && i < r && j > 0 && j < c){
        ...
        //the first column of the tile will load the halos on the left and on the right of the tile
        if(threadIdx.x == 0){
            //left
            Sz_shared[(threadIdx.y +1)* dim_buffers] = Sz[i * c + j -1];
            Sh_shared[(threadIdx.y +1)* dim_buffers] = Sh[i * c + j -1];
            //right
            Sz_shared[(threadIdx.y +1) * dim_buffers + TILE_SIZE +1] = Sz[i * c + j + TILE_SIZE];
            Sh_shared[(threadIdx.y +1) * dim_buffers + TILE_SIZE +1] = Sh[i * c + j + TILE_SIZE];  
        }
        //the first row of the tile will load the halos on top and bottom of the tile
        if(threadIdx.y == 0){
            //top
            Sz_shared[threadIdx.x +1] = Sz[(i -1)* c + j];
            Sh_shared[threadIdx.x +1] = Sh[(i -1)* c + j];
            //bottom
            Sz_shared[threadIdx.x + dim_buffers * (TILE_SIZE +1) +1] = Sz[(i + TILE_SIZE)* c + j];
            Sh_shared[threadIdx.x + dim_buffers * (TILE_SIZE +1) +1] = Sh[(i + TILE_SIZE)* c + j];  
        }
        if(threadIdx.x == 0 && threadIdx.y == 0){
            //top corners
            Sz_shared[0] = Sz[(i -1)* c + j -1];
            Sh_shared[0] = Sh[(i -1)* c + j -1];

            Sz_shared[dim_buffers -1] = Sz[(i -1) * c + j + TILE_SIZE];
            Sh_shared[dim_buffers -1] = Sh[(i -1) * c + j + TILE_SIZE];
            
            //bottom corners
            Sz_shared[(dim_buffers -1) * dim_buffers] = Sz[(i + TILE_SIZE) * c + j -1];
            Sh_shared[(dim_buffers -1) * dim_buffers] = Sh[(i + TILE_SIZE) * c + j -1];

            Sz_shared[dim_buffers * dim_buffers -1] = Sz[(i + TILE_SIZE) * c + j + TILE_SIZE];
            Sh_shared[dim_buffers * dim_buffers -1] = Sh[(i + TILE_SIZE) * c + j + TILE_SIZE];

        }
    }//else{} ghost cells are not needed since boundaries are not computed
    //
    __syncthreads();
